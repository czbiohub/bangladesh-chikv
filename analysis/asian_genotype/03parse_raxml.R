...
mltree_raw <- 
  read.nexus("../../data/ncbi_chrf_aln.fasta.raxml.bstree")
mltree_raw$node.label <- 
  slice(read.beast("../../data/ncbi_chrf_aln.fasta.raxml.bstree")@data, order(node))$bs
output$mltree_raw <- mltree_raw %<>% root(., sapply(outgroup_names, grep, .$tip.label))
strsplit(mltree_raw$tip.label, "_") %>%
  sapply(function (x) {
    if (grepl("CHRF", x)) return (paste(x, collapse="_"))
...
mltree_asian_df <- data.frame(seq=filter(mltree_asian_plot$data, isTip)$label,
  ...
  mutate(countries=strsplit(seq, "_")%>%sapply(`[`, 2)%>%
           ...
           gsub("ofthe", " of the", .))
mltree_asian_df[grep("CHRF", mltree_asian_df$seq), "countries"] <- "Bangladesh"
...
output$mltree_southasian <- mltree_southasian <-
  extract.clade(mltree_asian, 243)
...
seqinr::write.fasta(apply(sequences, 1, paste, collapse="")%>%as.list, names=rownames(sequences), 
                    file.out="../../data/ncbi_chrf_aln_southasian.fasta", as.string=TRUE)
output$partition_text <- partition_text <- readLines("../../data/partition_beast.txt")
